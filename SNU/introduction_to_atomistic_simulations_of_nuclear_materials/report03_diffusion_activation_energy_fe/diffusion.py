...
Na = 6.02214e23 # mol^-1
R = kB*Na # eV/(K*mol)
print(f'D0: {np.exp(lnD0):.5f} ' + r'Å^2/ps, ' + f'{(np.exp(lnD0)*(1e-10)**2/1e-12/1e-12):.3f}e-12 m^2/s')
print(f'E: {-E_R*R/Na:.4f} ' + r'eV per atom, ' + f'{-E_R*R*1.602e-19/1e3:.3f} kJ/mol')
fig, ax = plt.subplots(figsize=(8,6))
ax.scatter(1/T, np.log(D), label=r'$\rm ln\left( D(T) \right)$')
ax.plot(1/T, 1/T*E_R+lnD0, linewidth=2, label='Fit')
