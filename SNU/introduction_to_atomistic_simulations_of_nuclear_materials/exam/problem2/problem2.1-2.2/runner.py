...
for s in sc:
    ...
    os.system(f'/home/c2020spring/share/lammps-data.exe ../fcc-Cu.txt {s} {s} {s} > {perf}')
    ...
    with open(perf, 'r') as r:
        ...
        with open(f'{filn}-SIA110', 'w') as f:
            for i, line in enumerate(ls):
                if i==2:
                    N = int(line.split(' ')[0])
                    f.write(f'{N+1}  atoms\n')
                    ...
                    delta = 0.5
                    f.write(f'{N} 1 {coord-delta} {coord-delta} {coord}\n')
                    f.write(f'{N+1} 1 {coord+delta} {coord+delta} {coord}\n')
        # SIA - modify last line and add one line
        with open(f'{filn}-SIA111', 'w') as f:
            for i, line in enumerate(ls):
                if i==2:
                    N = int(line.split(' ')[0])
                    f.write(f'{N+1}  atoms\n')
