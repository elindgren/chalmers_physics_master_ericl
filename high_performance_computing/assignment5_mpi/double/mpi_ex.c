...
void print_grid(double* h_old, int nx, int ny){
  for(int i=0; i<ny; i++){
    for(int j=0; j<nx; j++)
...
static inline
void propagateOneTimeStep1(double* ht, double* htp, int nx, int ny, float d, float c){
...
static inline
void propagateOneTimeStep2(double* ht, double* htp, int nx, int ny, float d, float c,int up){
  for(int i=0; i<ny; i++){
    for(int j=1; j<nx-1; j+=2){
      int idx = (i+up)*nx + j;
      int idx1 = idx+1;
...
int main(int argc, char** argv){
  ...
  int scatter_root = 0;
  int up = 5; // Amount of extra rows that need to be included for unrolling
  ...
  if(mpi_rank == 0){
    ...
    h_old = (double*) malloc(sizeof(double)*nx*ny);
    h_new = (double*) malloc(sizeof(double)*nx*ny);
    for(int i = 0; i<ny; i++){
      for(int j = 0; j<nx ;j++){
	h_old[i*nx + j] = 0;
	h_new[i*nx + j] = 0;
  ...
  double* h_old_sub = (double*) malloc(sizeof(double)*lens[mpi_rank]);
  double* h_tmp_sub = (double*) malloc(sizeof(double)*lens[mpi_rank]);
  double* h_new_sub = (double*) malloc(sizeof(double)*lens_ret[mpi_rank]);
  ...
  for(int ix = 0; ix<nbr_iterations;ix+=up){
    ...
    //propagateOneTimeStep(h_old_sub, h_new_sub, nx, lens[mpi_rank]/nx, d_red, c);
